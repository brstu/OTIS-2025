...
int main(int argc, char** argv) {
    ...
    // Профиль задания: ступенька 0 -> 80 (начиная с k=20)
    auto w_func = [](int k) -> double {
        if (k < 20) return 0.0;
        return 80.0;
    };

    // Линейная модель: y[k] = a*y[k-1] + b*u[k-1]
    double a = 0.95;
    double b = 0.05;

    auto plant_step_linear = [a, b](int k, double u_k, double y_k1, double /*y_k2*/) -> double {
        // важно: в нашей sim мы используем u[k] для вычисления y[k],
        // здесь можно аппроксимировать как зависимость от u[k-1]; выберем простую рекурсию:
        return a * y_k1 + b * u_k;
    };
