...
TEST(PIDCoeffsTest, ComputeQ) {
    ...
    // q1 = -K*(1 + 2*TD/T0 - T0/T) = -2*(1 + 2*10 - 0.01) = -2*(1 + 20 - 0.01) = -2*20.99
    EXPECT_NEAR(q1, -2.0 * (1.0 + 2.0 * (1.0 / 0.1) - (0.1 / 10.0)), 1e-9);
}

TEST(PIDStepTest, SaturationAntiWindup) {
    PIDConfig cfg;
    cfg.K = 100.0; // большой K, чтобы быстро выйти на насыщение
    cfg.T = 10.0;
    cfg.TD = 0.0;
    cfg.T0 = 0.1;
    cfg.u_max = 1.0; // жесткая сатурация
    ...
    double u = 0.0;
    for (int k = 0; k < 100; ++k) {
        u = pid.step(1.0); // постоянная ошибка
