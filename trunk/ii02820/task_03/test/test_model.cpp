...
#include <cmath>
#include <vector>  // ← добавьте этот include
...
TEST(PIDTest, CoefficientsCalculation) {
    ...
    // ИСПРАВЛЕНО: T0 третьим, Td четвёртым
    PID pid(K, T, T0, Td);
...
TEST(PIDTest, SequentialCalculations) {
    // ИСПРАВЛЕНО: (1.0, 1.0, 0.1, 0.1) → T0=0.1, Td=0.1
...
TEST(PIDSystemTest, SystemStabilization) {
    // ИСПРАВЛЕНО: (1.5, 3.0, 1.0, 0.2) → T0=1.0, Td=0.2
    PID pid(1.5, 3.0, 1.0, 0.2);
...
TEST(PIDSystemTest, ConvergenceTest) {
    // ИСПРАВЛЕНО: (0.8, 2.0, 0.5, 0.1) → T0=0.5, Td=0.1
    PID pid(0.8, 2.0, 0.5, 0.1);
...
TEST(PIDTest, ConstantError) {
    // ИСПРАВЛЕНО: (1.0, 2.0, 1.0, 0.5) → T0=1.0, Td=0.5
    PID pid(1.0, 2.0, 1.0, 0.5);
