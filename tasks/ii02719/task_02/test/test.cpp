...
TEST(ComputeLinearTest, BasicCalculation) {
    int N = 3;
    double a = 1.5;// можно поменять чтобы сломать на a=0.6 тогда y[1]=1.6 а тест ожидает 1.5
    double b = 1.0;
...
TEST(ComputeLinearTest, SingleStep) {
    ...
    double a = 1.0;
    double b = 2.0;
    std::vector<double> u = {4.0};// можно поменять на 
    std::vector<double> y_lin(N + 1, 0.0);
    y_lin[0] = 10.0;
...
TEST(ComputeLinearTest, ZeroSteps) {
    ...
    double b = 1.0;
    std::vector<double> u = {};
    std::vector<double> y_lin(3, 10.0); // y[0] = 10.0
