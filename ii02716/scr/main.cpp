#include <iostream>
#include <cmath>
#include <limits>  // Добавлено для clear()
...
public:
    ...
    float calculateNext(float y1) const {  // Добавлен const
        return a * y1 + b * u;
    }
};
...
public:
    ...
    float calculateNext(float y1, float y0) const {  // Добавлен const
        return a * y1 - b * y0 * y0 + c * u + d * sin(u);
    }
};

// Функция для вывода результатов
void printResult(int iteration, float value) {  // Убран неиспользуемый параметр
    cout << " t" << iteration + 1 << " = " << value << endl;
}
...
int main() {
    ...
    // Ввод начального условия с проверкой
